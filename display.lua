...
local function commas(n)local s=tostring(math.floor(math.abs(n)+.5));local r=string.reverse(s);r=string.gsub(r,"(%d%d%d)","%1,");r=string.reverse(r);r=string.gsub(r,"^,","");return (n<0 and "-" or "")..r end
...
local function waiting(msg)mon.setBackgroundColor(C.black);mon.clear();center(math.floor(H/2)-2,string.upper(cfg.displayName),C.purple);center(math.floor(H/2),string.upper(cfg.subtitle),C.cyan);center(math.floor(H/2)+3,msg,C.red)end
...
local function draw()
 ...
 center(1,string.upper(cfg.displayName).."  //  "..string.upper(cfg.subtitle),C.purple);center(2,"TIER "..tier.." DRACONIC ENERGY CORE",C.lightGray);center(3,"<<<  SYSTEM "..state.."  >>>",sc);line(4,1,W,"=",C.lightGray)
