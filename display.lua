...
local function loadConfig()
 ...
 -- migrate earlier config formats instead of crashing on missing fields
 if type(c.displayName)~="string" or c.displayName=="" then
  if type(c.name)=="string" and c.name~="" then c.displayName=c.name else return nil end
 end
 c.subtitle=nil
 saveConfig(c)
 return c
...
local function askName(oldName)
 ...
 local n=read()
 if n=="" then n=oldName or "ENERGY CORE" end
 local c={displayName=n};saveConfig(c);return c
end

local args={...}
local cfg=loadConfig()
if args[1]=="config" then cfg=askName(cfg and cfg.displayName or nil) elseif not cfg then cfg=askName(nil) end
-- final defensive fallback: display code must never receive nil here
...
local function waiting(msg)mon.setBackgroundColor(C.black);mon.clear();center(math.floor(H/2)-2,string.upper(cfg.displayName),C.purple);center(math.floor(H/2),"DRACONIC ENERGY CORE",C.cyan);center(math.floor(H/2)+3,msg,C.red)end
...
local function draw()
 ...
 local etaFull=an>0 and ((cap-stored)/an)/20 or nil;local etaEmpty=an<0 and (stored/math.abs(an))/20 or nil;local uptime=(now-startMs)/1000;local tier=tierFor(cap);local utilization=flowMax>0 and math.abs(net)/flowMax or 0;local ioRatio=input>0 and output/input or 0;local reserve=math.max(0,cap-stored)
 mon.setBackgroundColor(C.black);mon.clear()
 center(1,string.upper(cfg.displayName),C.purple);center(2,"TIER "..tier.." DRACONIC ENERGY CORE",C.lightGray);center(3,"<<<  SYSTEM "..state.."  >>>",sc);line(4,1,W,"=",C.lightGray)
 ...
 put(a+4,7,"STORED",C.lightGray);put(a+18,7,fmt(stored,4).." OP",C.yellow);put(b-30,7,"CAPACITY",C.lightGray);put(b-18,7,fmt(cap,4).." OP",C.yellow);segmented(a+4,10,b-a-10,pct,C.lime);put(b-15,12,string.format("%9.5f%%",pct*100),C.white);line(14,a+4,b-4,"-",C.gray);put(a+4,15,"0%",C.gray);put(math.floor((a+b)/2)-2,15,"50%",C.gray);put(b-8,15,"100%",C.gray)
 ...
 local r2t=18;local r2b=31;local flowR=math.floor(W*.70);box(1,r2t,flowR,r2b,"POWER FLOW (OP/t)");box(flowR+1,r2t,W,r2b,"POWER FLOW STATUS");local gx=30;local gw=math.max(15,flowR-gx-12);local rows={{"INPUT","+",input,C.lime},{"OUTPUT","-",output,C.orange},{"NET",net>=0 and "+" or "-",math.abs(net),C.lightBlue}};for i,r in ipairs(rows)do local y=r2t+2+i*3;put(3,y,string.format("%-7s %s%10s",r[1],r[2],fmt(r[3],2)),r[4]);bar(gx,y,gw,r[3],flowMax,r[4],C.gray);put(flowR-10,y,fmt(flowMax,0),C.lightGray)end
 ...
 line(H,1,W,"=",C.lightGray);put(2,H,"LIVE TELEMETRY // RF 10 Hz // DISPLAY 20 Hz",C.gray);local clock=textutils.formatTime(os.time(),true);put(math.floor(W/2)-math.floor(#clock/2),H,clock,C.yellow);put(math.max(2,W-28),H,"SEQ #"..tostring(packet.sequence or 0),C.yellow)
end

local function receiver()while true do local _,msg=rednet.receive(PROTOCOL);if type(msg)=="table" and msg.version==1 and type(msg.stored)=="number" then packet=msg;lastPacketMs=os.epoch("utc");rxCount=rxCount+1;samples[#samples+1]={t=lastPacketMs,input=tonumber(msg.input) or 0,output=tonumber(msg.output) or 0,net=tonumber(msg.net) or ((tonumber(msg.input) or 0)-(tonumber(msg.output) or 0))};history[#history+1]=tonumber(msg.net) or 0;if #history>120 then table.remove(history,1)end end end end
